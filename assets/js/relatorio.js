window.ZARelatorio = (() => {
  function renderScoreBars(lead) {
    ...
    return pillars.map(p => {
      const status = window.ZACalculos.statusLabel(p.score);
      return `
        <div class="score-row">
          ...
          <span class="chip ${status.className}" style="color:${status.color}; border-color:#e8edf3;">${status.label}</span>
        </div>
      `;
    }).join("");
  }
    ...
    return pillars.map(p => {
      ...
      return `
        <div class="pillar-card">
          ...
          <p>${window.ZACalculos.pillarTexts[p.key]}</p>
        </div>
      `;
    }).join("");
  }
  ...
  function drawRadar(canvasId, lead) {
    ...
    const values = [
      lead.score_movimento,
      lead.score_alimentacao,
      lead.score_sono,
      lead.score_proposito,
      lead.score_social,
      lead.score_estresse
    ];

    const pointColors = values.map(v => window.ZACalculos.statusLabel(v).color);
  ...
  function renderReport(email) {
    const lead = window.ZAStorage.findLeadByEmail(email);
    ...
    const profile = window.ZACalculos.profileLabelByMedia(Number(lead.media_geral));
    ...
    const pilarMaisBaixo = lead.pilar_mais_baixo ? {
      ...
      text: window.ZACalculos.pillarTexts[lead.pilar_mais_baixo]
    } : null;

    const introText = "As Zonas Azuis são regiões do mundo onde as pessoas vivem mais e melhor. Em comum, elas compartilham padrões consistentes de movimento, alimentação, propósito, vínculos sociais, sono e manejo do estresse.";

    reportContent.innerHTML = `
      <div class="report-head">
        <h1>Relatório Zona Azul</h1>
      ...
      <div class="report-body">
              ...
              ? `<div class="report-grid three">
                  ${gaps.slice(0,3).map(g => `
                    <div class="report-kpi">
                      ...
                      <strong style="color:${g.cor};">${g.score}/10</strong>
                    </div>
                  `).join("")}
                </div>`
        ...
        <div class="report-section">
          <h3>Próximo passo</h3>
          <p>O próximo nível desse processo é o <strong>Diagnóstico Completo Zona Azul</strong>, onde aprofundamos os padrões do seu caso, definimos prioridades de atuação e estruturamos um plano mais preciso para saúde, qualidade de vida e longevidade.</p>
        </div>
      </div>
    `;
