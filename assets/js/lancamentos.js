window.ZALancamentos = (() => {
  ...
  function renderSessaoEAvaliacao() {
    const bloco = cliente?.sessaoEAvaliacao || {};
    const sessao = bloco.sessao || {};
    const a = bloco.avaliacao || {};
    ...
    const updatedEl = document.getElementById("sessao-avaliacao-updated");
    if (updatedEl) {
      updatedEl.textContent = `Última atualização: ${cliente?.sessaoEAvaliacaoUpdatedAt ? formatDate(cliente.sessaoEAvaliacaoUpdatedAt) : "—"}`;
    }
  }
  ...
  function renderAnaliseCaso() {
    ...
    const bloco = cliente?.analiseCaso || {};
    const radar = bloco.radarRevisado || {};
    const d = bloco.diagnosticoCompleto || {};

    RADAR_KEYS.forEach((key) => {
      setText(`radar-pre-${key}`, radarPre?.[key] ?? "—");
      setValue(`radar-revisado-${key}`, radar?.[key] ?? "");
    });
  ...
  function saveAnaliseCaso() {
    ...
    let clienteAtualizado = {
      ...
      analiseCasoUpdatedAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
